resource "digitalocean_domain" "dbm2" {
    name = "dbm2.do.lab.seqvence.com"
...
resource "null_resource" "dbm2" {
  ...
  connection {
      ...
      timeout = "6m"
      host = "${digitalocean_droplet.dbm2.ipv4_address}"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-playbook -i \"localhost,\" -c local --extra-vars '{\"INTERNAL_IP\":\"${digitalocean_droplet.dbm2.ipv4_address_private}\", \"CONSUL_ENDPOINT\":\"${digitalocean_droplet.dbm2.ipv4_address_private}\", \"HOSTNAME\":\"dbm2\", \"BOOTSTRAP\": false, \"CONSUL_ENDPOINT\":\"${digitalocean_droplet.dbm0.ipv4_address_private}\", \"JOIN\": true}' /root/local_playbook.yml",
