resource "digitalocean_domain" "dbs1" {
    name = "dbs1.do.lab.seqvence.com"
    ip_address = "${digitalocean_droplet.dbs1.ipv4_address}"
}

resource "digitalocean_droplet" "dbs1" {
...
resource "null_resource" "dbs1" {
  ...
  connection {
      ...
      timeout = "6m"
      host = "${digitalocean_droplet.dbs1.ipv4_address}"
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "ansible-playbook -i \"localhost,\" -c local --extra-vars '{\"INTERNAL_IP\":\"${digitalocean_droplet.dbs1.ipv4_address_private}\", \"CONSUL_ENDPOINT\":\"${digitalocean_droplet.dbm0.ipv4_address_private}\", \"HOSTNAME\":\"dbs1\"}' /root/local_playbook.yml",
